...
public class JdotxtTaskList extends JPanel implements Scrollable {
    ...
    public void setPrependMetadata(boolean prependMetadata) {
        ...
        if (prependMetadata) {
            if (metaData.isEmpty()) {
                emptyTaskString = newTask;
            } else {
                emptyTaskString = metaData + " " + newTask;
            }
        } else {
            if (metaData.isEmpty()) {
                emptyTaskString = newTask;
            } else {
                emptyTaskString = newTask + " " + metaData;
            }
        }
    ...
    public void requestFocusNewTask() {
        if (newTaskPanel.isVisible()) {
            newTaskPanel.requestFocus(JdotxtTaskPanel.CONTENT);
        }
    }
    ...
    private boolean isUpToDate(List<Task> tasks) {
        ...
        for (int k1 = 0; inOrder && k1 < tasks.size(); k1++) {
            inOrder = (tasks.get(k1).equals(this.tasks.get(k1)));
        }
        return inOrder;
    }
        ...
        public void run() {
            ...
            if (!isInterrupted()) {
                if (postProcessing != null) {
                    EventQueue.invokeLater(postProcessing);
                }
            }
        }
    }
    ...
    private GUItask getGUItask(Task t) {
        for (GUItask gt: guiTaskPanelList) {
            if (gt.task == t) {
                return gt;
            }
        }
    ...
    private class TasksPanelListener implements TaskListener {
        ...
        public void taskUpdated(Task t, short field) {
            ...
            for (int i = taskListenerList.size()-1; i >= 0; i--) {
                taskListenerList.get(i).taskUpdated(t, field);
            }
        ...
        public void taskDeleted(Task t) {
            ...
            repaint();
            for (int i = taskListenerList.size()-1; i >= 0; i--) {
                taskListenerList.get(i).taskDeleted(t);
            }
        }
        ...
        public void enterPressed(Task t, short field) {
            ...
            updateTaskList(t, field);
            for (int i = taskListenerList.size()-1; i >= 0; i--) {
                taskListenerList.get(i).enterPressed(t, field);
            }
        }
        ...
        private void updateTaskList(Task t, short field) {
            ...
            if (!isUpToDate(tasks)) {
                ...
                if (field == JdotxtTaskPanel.COMPLETED && t.isCompleted()) {
                    ...
                    postprocessing = new RequestFocus(t, field);
                }
                forceUpdateTaskList(tasks, postprocessing);
            }
        }
	...
	private class NewTaskPanelListener implements TaskListener {
		...
		public void taskCreated(Task t) {
			addNewTask(t);
			for (int i = taskListenerList.size()-1; i >= 0; i--) {
                taskListenerList.get(i).taskCreated(t);
            }
		}
		@Override
		public void taskUpdated(Task t, short field) {
